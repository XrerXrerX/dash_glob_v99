...
<div class="sec_table">
    ...
    <div class="group_act_butt">
        <a href="#" onclick="handleButtonAddClick(this.id)" id="add-mediastream">
        ...
        <div class="all_act_butt">
            <a href="#" onclick="handleButtonEditClick(this.id)" id="update-mediastream">
                <div class="sec_edit">
        ...
        <tbody>
            ...
            @foreach ($data as $index => $d)
                <tr>
                    ...
                    <td><span class="name">{{ $d->strm_nama }}</span></td>
                    <td class="show_img_tbl">
                        <span class="td_img_show gambar">{{ substr($d->strm_foto, 0, 15) }}</span>
                        <img class="table_img" src="{{ asset('storage/' . $d->strm_foto) }}" alt=""
                            style="display: none;">
                    </td>
                    <td><span class="name">{{ $d->strm_youtube }}</span></td>
                    <td><span class="name">{{ $d->strm_facebook }}</span></td>
                    <td><span class="name">{{ $d->strm_instagram }}</span></td>
                    <td><span class="name">{{ $d->strm_tiktok }}</span></td>
                    <td class="show_img_tbl">
                        <span class="td_img_show gambar">{{ substr($d->bannerup, 0, 15) }}</span>
                        <img class="table_img" src="{{ asset('storage/' . $d->bannerup) }}" alt=""
                            style="display: none;">
                    </td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#update-mediastream').off('click').click(function(event) {
            ...
            $('.aplay_code').load('/web/mediastream/edit/' + parameterString, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediastream/edit/' + parameterString);
            });
        });


        $(document).off('click', '#add-mediastream').on('click', '#add-mediastream', function(event) {
            event.preventDefault();
            $('.aplay_code').load('/web/mediastream/add', function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediastream/add');
            });
        });
        // $(document).on('click', '#delete', function(event) {
        //     event.preventDefault();
        //     $('.aplay_code').load('/web/mediastream/delete', function() {
        //         adjustElementSize();
        //         localStorage.setItem('lastPage', '/web/mediastream/delete');
        //     });
        // })



        $(document).on('click', '#delete-mediastream', function(event) {
            event.preventDefault();
            ...
            var url = "/web/mediastream/delete/"; // Ubah URL sesuai dengan endpoint delete yang sesuai
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            Swal.fire({
                                ...
                                $('.aplay_code').load('/web/mediastream',
                                    function() {
                                        adjustElementSize();
        ...
        $(document).off('click', '#view').on('click', '#view', function(event) {
            ...
            var id = $(this).data('id');
            $('.aplay_code').empty();
            $('.aplay_code').load('/web/mediastream/view/' + id, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediastream/view/' + id);
            });
        });
        ...
        $(document).off('click', '#edit').on('click', '#edit', function(event) {
            ...
            var id = $(this).data('id');
            $('.aplay_code').empty();
            $('.aplay_code').load('/web/mediastream/edit/' + id, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediastream/edit/' + id);
            });
        });
