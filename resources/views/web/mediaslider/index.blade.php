...
<div class="sec_table">
    <h2>{{ $title }}</h2>
    {{-- <a href="#" onclick="handleButtonAddClick(this.id)" id="add-mediaslider">
    ...
    <a href="#" onclick="handleButtonEditClick(this.id)" id="update-mediaslider">
        <div class="sec_edit">
        ...
        <tbody>
            ...
            @foreach ($data as $index => $d)
                <tr>
                    ...
                    <td><span class="name">{{ $d->car_news }}</span></td>
                    <td><span class="name">{{ $d->car_event }}</span></td>
                    <td><span class="name">{{ $d->car_stream }}</span></td>
                    <td><span class="name">{{ $d->car_panduan }}</span></td>
                    <td><span class="name">{{ $d->car_gallery }}</span></td>
                    <td><span class="name">{{ $d->card_news }}</span></td>
                    <td><span class="name">{{ $d->card_event }}</span></td>
                    <td><span class="name">{{ $d->card_stream }}</span></td>
                    <td><span class="name">{{ $d->card_panduan }}</span></td>
                    <td><span class="name">{{ $d->card_gallery }}</span></td>
    ...
    $(document).ready(function() {
        ...
        $('#myCheckbox, [id^="myCheckbox-"]').change(function() {
            ...
            if (isChecked) {
                ...
                $('#update-mediaslider').css('display', 'flex');
            } else {
                $('.all_act_butt').hide();
                $('#update-mediaslider').hide();
            }
        ...
        $('#update-mediaslider').off('click').click(function(event) {
            ...
            $('.aplay_code').load('/web/mediaslider/edit/' + parameterString, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediaslider/edit/' + parameterString);
            });
        });


        $(document).off('click', '#add-mediaslider').on('click', '#add-mediaslider', function(event) {
            event.preventDefault();
            $('.aplay_code').load('/web/mediaslider/add', function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediaslider/add');
            });
        });
        // $(document).on('click', '#delete', function(event) {
        //     event.preventDefault();
        //     $('.aplay_code').load('/web/mediaslider/delete', function() {
        //         adjustElementSize();
        //         localStorage.setItem('lastPage', '/web/mediaslider/delete');
        //     });
        // })



        $(document).on('click', '#delete-mediaslider', function(event) {
            event.preventDefault();
            ...
            var url = "/web/mediaslider/delete/"; // Ubah URL sesuai dengan endpoint delete yang sesuai
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            Swal.fire({
                                ...
                                $('.aplay_code').load('/web/mediaslider',
                                    function() {
                                        adjustElementSize();
        ...
        $(document).off('click', '#view').on('click', '#view', function(event) {
            ...
            var id = $(this).data('id');
            $('.aplay_code').empty();
            $('.aplay_code').load('/web/mediaslider/view/' + id, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediaslider/view/' + id);
            });
        });
        ...
        $(document).off('click', '#edit').on('click', '#edit', function(event) {
            ...
            var id = $(this).data('id');
            $('.aplay_code').empty();
            $('.aplay_code').load('/web/mediaslider/edit/' + id, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediaslider/edit/' + id);
            });
        });
