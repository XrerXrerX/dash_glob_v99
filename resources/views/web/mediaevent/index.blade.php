...
<div class="sec_table">
    ...
    <div class="group_act_butt">
        <a href="#" onclick="handleButtonAddClick(this.id)" id="add-mediaevent">
        ...
        <div class="all_act_butt">
            <a href="#" onclick="handleButtonEditClick(this.id)" id="update-mediaevent">
                <div class="sec_edit">
        ...
        <tbody>
            ...
            @foreach ($data as $index => $d)
                <tr>
                    ...
                    <td><span class="name">{{ $d->nama_event }}</span></td>
                    <td class="show_img_tbl">
                        <span class="td_img_show gambar">{{ substr($d->gambar_event, 0, 15) }}</span>
                        <img class="table_img" src="{{ asset('storage/' . $d->gambar_event) }}" alt=""
                            style="display: none;">
                    </td>
                    <td><span class="name">{{ $d->durasi_event }}</span></td>
                    <td><span class="name">{{ $d->desk_event }}</span></td>
                    <td><span class="name">{{ date('d-m-Y H:i', strtotime($d->tgl_event)) }}</span></td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#update-mediaevent').off('click').click(function(event) {
            ...
            $('.aplay_code').load('/web/mediaevent/edit/' + parameterString, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediaevent/edit/' + parameterString);
            });
        });


        $(document).off('click', '#add-mediaevent').on('click', '#add-mediaevent', function(event) {
            event.preventDefault();
            $('.aplay_code').load('/web/mediaevent/add', function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediaevent/add');
            });
        });
        // $(document).on('click', '#delete', function(event) {
        //     event.preventDefault();
        //     $('.aplay_code').load('/web/mediaevent/delete', function() {
        //         adjustElementSize();
        //         localStorage.setItem('lastPage', '/web/mediaevent/delete');
        //     });
        // })



        $(document).on('click', '#delete-mediaevent', function(event) {
            event.preventDefault();
            ...
            var url = "/web/mediaevent/delete/"; // Ubah URL sesuai dengan endpoint delete yang sesuai
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            Swal.fire({
                                ...
                                $('.aplay_code').load('/web/mediaevent',
                                    function() {
                                        adjustElementSize();
        ...
        $(document).off('click', '#view').on('click', '#view', function(event) {
            ...
            var id = $(this).data('id');
            $('.aplay_code').empty();
            $('.aplay_code').load('/web/mediaevent/view/' + id, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediaevent/view/' + id);
            });
        });
        ...
        $(document).off('click', '#edit').on('click', '#edit', function(event) {
            ...
            var id = $(this).data('id');
            $('.aplay_code').empty();
            $('.aplay_code').load('/web/mediaevent/edit/' + id, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediaevent/edit/' + id);
            });
        });
