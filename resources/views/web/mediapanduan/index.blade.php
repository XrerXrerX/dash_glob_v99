...
<div class="sec_table">
    ...
    <div class="group_act_butt">
        <a href="#" onclick="handleButtonAddClick(this.id)" id="add-mediapanduan">
        ...
        <div class="all_act_butt">
            <a href="#" onclick="handleButtonEditClick(this.id)" id="update-mediapanduan">
                <div class="sec_edit">
        ...
        <tbody>
            ...
            @foreach ($data as $index => $d)
                <tr>
                    ...
                    <td><span class="name">{{ $d->panduan_judul }}</span></td>
                    <td class="show_img_tbl">
                        <span class="td_img_show gambar">{{ substr($d->panduan_gambar, 0, 15) }}</span>
                        <img class="table_img" src="{{ asset('storage/' . $d->panduan_gambar) }}" alt=""
                            style="display: none;">
                    </td>
                    <td><span class="name">{{ $d->p1080_panduan }}</span></td>
                    <td><span class="name">{{ $d->panduan_desk }}</span></td>
                    <td><span class="name">{{ date('d-m-Y H:i:s', strtotime($d->panduan_tgl)) }}</span></td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#update-mediapanduan').off('click').click(function(event) {
            ...
            $('.aplay_code').load('/web/mediapanduan/edit/' + parameterString, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediapanduan/edit/' + parameterString);
            });
        });


        $(document).off('click', '#add-mediapanduan').on('click', '#add-mediapanduan', function(event) {
            event.preventDefault();
            $('.aplay_code').load('/web/mediapanduan/add', function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediapanduan/add');
            });
        });
        // $(document).on('click', '#delete', function(event) {
        //     event.preventDefault();
        //     $('.aplay_code').load('/web/mediapanduan/delete', function() {
        //         adjustElementSize();
        //         localStorage.setItem('lastPage', '/web/mediapanduan/delete');
        //     });
        // })



        $(document).on('click', '#delete-mediapanduan', function(event) {
            event.preventDefault();
            ...
            var url = "/web/mediapanduan/delete/"; // Ubah URL sesuai dengan endpoint delete yang sesuai
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            Swal.fire({
                                ...
                                $('.aplay_code').load('/web/mediapanduan',
                                    function() {
                                        adjustElementSize();
        ...
        $(document).off('click', '#view').on('click', '#view', function(event) {
            ...
            var id = $(this).data('id');
            $('.aplay_code').empty();
            $('.aplay_code').load('/web/mediapanduan/view/' + id, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediapanduan/view/' + id);
            });
        });
        ...
        $(document).off('click', '#edit').on('click', '#edit', function(event) {
            ...
            var id = $(this).data('id');
            $('.aplay_code').empty();
            $('.aplay_code').load('/web/mediapanduan/edit/' + id, function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/web/mediapanduan/edit/' + id);
            });
        });
