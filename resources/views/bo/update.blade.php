        ...
        @foreach ($data as $index => $item)
            <div class="sec_form">
                ...
                <div class="list_form">
                    ...
                    <input type="text" id="nama" name="nama[]" value="{{ $item->nama }}" {{ $disabled }}>
                    <input type="hidden" name="id[]" value="{{ $item->id }}">
                </div>

                <div class="list_form">
                    <span class="sec_label">Site</span>
                    <input type="text" id="site" name="site[]" value="{{ $item->site }}" {{ $disabled }}>
                </div>
            </div>
        @endforeach
...
<script>
    $(document).ready(function() {
        ...
        $(document).off('click', '#cancel').on('click', '#cancel', function(event) {
            ...
            $('.aplay_code').load('/apk/bo', function() {
                adjustElementSize();
                localStorage.setItem('lastPage', '/apk/bo');
            });
        });
    });
</script>
