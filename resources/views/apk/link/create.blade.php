    ...
    <form action="" method="POST" enctype="multipart/form-data" id="linkForm">
        ...
        @foreach ($data as $index => $item)
            <div class="sec_form">
                ...
                @foreach ($item as $indx => $itms)
                    @foreach ($itms as $ind => $itm)
                        <div class="list_form">
                            <span class="sec_label">Link </span>
                            <input type="hidden" name="id[]" value="{{ $itm['id'] }}">
                            <input type="hidden" name="namabo[]" value="{{ $namabo[0] }}">
                            <input type="text" id="link" name="link[]" value="{{ $itm['link'] }}"
                                {{ $disabled }} placeholder="Masukkan Link Alternatif 1">
                        </div>
                    @endforeach
                @endforeach
        @endforeach
