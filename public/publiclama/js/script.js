...
$(document).on('mouseenter', '.sec_sidebar', function() {
    if (isExpanded) {
    $('.gmb_logo').attr('src', function(index, oldSrc) {
        return oldSrc.replace('21icon.png', 'Logo-Lotto21.png');
    });
...
$(document).ready(function() {
    var maxVisibleRows = 20; // Jumlah maksimum baris yang ditampilkan secara default
  
    // Sembunyikan semua baris kecuali 20 yang pertama
    $('tr.filter_row').nextAll('tr').hide();
  
    var defaultVisibleRows = $('tr.filter_row').nextAll('tr').slice(0, maxVisibleRows);
    defaultVisibleRows.show();
  
    $('body').on('keyup', 'input[id^="searchData-"]', function() {
      var searchValue = $(this).val().toLowerCase().trim();
      var targetClass = $(this).attr('id').split('-')[1];
      var rows = $('tr.filter_row').nextAll('tr');
      var visibleRowCount = 0;
  
      rows.hide();
  
      rows.each(function() {
        var text = $(this).find('.' + targetClass).text().toLowerCase().trim();
        if (text.includes(searchValue)) {
          $(this).show();
          visibleRowCount++;
        }
      ...
      // Sembunyikan baris tambahan jika lebih dari 20 yang tampil
      var totalVisibleRows = Math.min(visibleRowCount, maxVisibleRows);
      if (visibleRowCount > maxVisibleRows) {
        rows.slice(0, totalVisibleRows).show();
      }

      if(searchValue == ''){
        $('tr.filter_row').nextAll('tr').hide();
        $('tr.filter_row').nextAll('tr').slice(0, 20).show();

      }
    });
  });

// Menampilkan 20 baris secara default
//   $('tr.filter_row').nextAll('tr').each(function(index) {
