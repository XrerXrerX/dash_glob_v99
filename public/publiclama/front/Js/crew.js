const sectionWebsites = document.querySelectorAll('.section.website');

sectionWebsites.forEach(section => {
  const partWebsites = section.querySelectorAll('.partwebsite');

  partWebsites.forEach(partWebsite => {
    const dataId = partWebsite.getAttribute('data-id');
    const gifBanner = partWebsite.querySelector('.gifbannerutama');
    const loader = partWebsite.querySelector('.kurungloadutama');

    gifBanner.addEventListener('load', () => {
      loader.style.display = 'none';
      gifBanner.style.display = 'block';
    });

    if (!gifBanner.complete) {
      loader.style.display = 'block';
      gifBanner.style.display = 'none';
    } else {
      loader.style.display = 'none';
      gifBanner.style.display = 'block';
    }
...
games.forEach((game) => {
const progressValue = Math.floor(Math.random() * (97 - 81 + 1)) + 81;
